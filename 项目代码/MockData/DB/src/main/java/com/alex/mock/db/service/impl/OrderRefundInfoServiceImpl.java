...
public class OrderRefundInfoServiceImpl extends ServiceImpl<OrderRefundInfoMapper, OrderRefundInfo> implements OrderRefundInfoService {
    ...
    RandomOptionGroup<String>  refundTypeOptionGroup=new RandomOptionGroup(new RanOpt(Constant.REFUND_TYPE_ONLY_MONEY,30),new RanOpt(Constant.REFUND_TYPE_WITH_GOODS,60) );
    ...
    public void  genRefundsOrFinish(Boolean ifClear){
        ...
        RandomOptionGroup<String>  refundReasonOptionGroup=new RandomOptionGroup(new RanOpt(Constant.REFUND_REASON_BAD_GOODS,refundReasonRateArr[0]),
                new RanOpt(Constant.REFUND_REASON_SIZE_ISSUE,refundReasonRateArr[1]),
                new RanOpt(Constant.REFUND_REASON_SALE_OUT,refundReasonRateArr[2]),
                new RanOpt(Constant.REFUND_REASON_MISTAKE,refundReasonRateArr[3]),
                new RanOpt(Constant.REFUND_REASON_WRONG_DESC,refundReasonRateArr[4]),
                new RanOpt(Constant.REFUND_REASON_NO_REASON,refundReasonRateArr[5]),
                new RanOpt(Constant.REFUND_REASON_OTHER,refundReasonRateArr[6])
        );

        QueryWrapper<OrderInfo> orderInfoQueryWrapper = new QueryWrapper<>();
        orderInfoQueryWrapper.in("order_status", Constant.ORDER_STATUS_PAID, Constant.ORDER_STATUS_FINISH);
        ...
        for (OrderInfo orderInfo : orderInfoList) {
            if( ifRefund.getRandBoolValue() ){
                ...
                orderInfo.setOrderStatus(Constant.ORDER_STATUS_REFUND);
